...
router.post('/vehicle_details', auth, async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Vehicle details updated successfully', vehicle: user.vehicle, vehicleType: user.vehicleType });
  } catch (error) {
...
router.get('/vehicle_details', auth, async (req, res) => {
  try {
    const user = await User.findById(req.userId);
    if (!user) {
      return res.status(404).json({ message: 'User not found' });
    ...
    res.json({ vehicle: user.vehicle, vehicleType: user.vehicleType });
  } catch (error) {
